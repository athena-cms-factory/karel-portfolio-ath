 ...
 *
 * @license MIT
 */function xc(){return xc=Object.assign?Object.assign.bind():function(y){for(var s=1;s<arguments.length;s++){var k=arguments[s];for(var d in k)Object.prototype.hasOwnProperty.call(k,d)&&(y[d]=k[d])}return y},xc.apply(this,arguments)}var O2;(function(y){y.Pop="POP",y.Push="PUSH",y.Replace="REPLACE"})(O2||(O2={}));const $z="popstate";function Bj(y){y===void 0&&(y={});function s(L,C){let{pathname:j="/",search:T="",hash:b=""}=Lc(L.location.hash.substr(1));return!j.startsWith("/")&&!j.startsWith(".")&&(j="/"+j),Vb("",{pathname:j,search:T,hash:b},C.state&&C.state.usr||null,C.state&&C.state.key||"default")}function k(L,C){let j=L.document.querySelector("base"),T="";if(j&&j.getAttribute("href")){let b=L.location.href,f=b.indexOf("#");T=f===-1?b:b.slice(0,f)}return T+"#"+(typeof C=="string"?C:nx(C))}function d(L,C){pA(L.pathname.charAt(0)==="/","relative pathnames are not supported in hash history.push("+JSON.stringify(C)+")")}return Rj(s,k,d,y)}function z1(y,s){if(y===!1||y===null||typeof y>"u")throw new Error(s)}function pA(y,s){if(!y){typeof console<"u"&&console.warn(s);try{throw new Error(s)}catch{}}}function Oj(){return Math.random().toString(36).substr(2,8)}function eA(y,s){return{usr:y.state,key:y.key,idx:s}}function Vb(y,s,k,d){return k===void 0&&(k=null),xc({pathname:typeof y=="string"?y:y.pathname,search:"",hash:""},typeof s=="string"?Lc(s):s,{state:k,key:s&&s.key||d||Oj()})}function nx(y){let{pathname:s="/",search:k="",hash:d=""}=y;return k&&k!=="?"&&(s+=k.charAt(0)==="?"?k:"?"+k),d&&d!=="#"&&(s+=d.charAt(0)==="#"?d:"#"+d),s}function Lc(y){let s={};if(y){let k=y.indexOf("#");k>=0&&(s.hash=y.substr(k),y=y.substr(0,k));let d=y.indexOf("?");d>=0&&(s.search=y.substr(d),y=y.substr(0,d)),y&&(s.pathname=y)}return s}function Rj(y,s,k,d){d===void 0&&(d={});let{window:L=document.defaultView,v5Compat:C=!1}=d,j=L.history,T=O2.Pop,b=null,f=D();f==null&&(f=0,j.replaceState(xc({},j.state,{idx:f}),""));function D(){return(j.state||{idx:null}).idx}function q(){T=O2.Pop;let X=D(),F=X==null?null:X-f;f=X,b&&b({action:T,location:ae.location,delta:F})}function V(X,F){T=O2.Push;let de=Vb(ae.location,X,F);k&&k(de,X),f=D()+1;let ue=eA(de,f),Q=ae.createHref(de);try{j.pushState(ue,"",Q)}catch(we){if(we instanceof DOMException&&we.name==="DataCloneError")throw we;L.location.assign(Q)}C&&b&&b({action:T,location:ae.location,delta:1})}function oe(X,F){T=O2.Replace;let de=Vb(ae.location,X,F);k&&k(de,X),f=D();let ue=eA(de,f),Q=ae.createHref(de);j.replaceState(ue,"",Q),C&&b&&b({action:T,location:ae.location,delta:0})}function Y(X){let F=L.location.origin!=="null"?L.location.origin:L.location.href,de=typeof X=="string"?X:nx(X);return de=de.replace(/ $/,"%20"),z1(F,"No window.location.(origin|href) available to create URL for href: "+de),new URL(de,F)}let ae={get action(){return T},get location(){return y(L,j)},listen(X){if(b)throw new Error("A history only accepts one active listener");return L.addEventListener($z,q),b=X,()=>{L.removeEventListener($z,q),b=null}},createHref(X){return s(L,X)},createURL:Y,encodeLocation(X){let F=Y(X);return{pathname:F.pathname,search:F.search,hash:F.hash}},push:V,replace:oe,go(X){return j.go(X)}};return ae}var tA;(function(y){y.data="data",y.deferred="deferred",y.redirect="redirect",y.error="error"})(tA||(tA={}));function kA(y,s){if(s==="/")return y;if(!y.toLowerCase().startsWith(s.toLowerCase()))return null;let k=s.endsWith("/")?s.length-1:s.length,d=y.charAt(k);return d&&d!=="/"?null:y.slice(k)||"/"}const _j=/^(?:[a-z][a-z0-9+.-]*:|\/\/)/i,Uj=y=>_j.test(y);function Ej(y,s){s===void 0&&(s="/");let{pathname:k,search:d="",hash:L=""}=typeof y=="string"?Lc(y):y,C;if(k)if(Uj(k))C=k;else{if(k.includes("//")){let j=k;k=k.replace(/\/\/+/g,"/"),pA(!1,"Pathnames cannot have embedded double slashes - normalizing "+(j+" -> "+k))}k.startsWith("/")?C=aA(k.substring(1),"/"):C=aA(k,s)}else C=s;return{pathname:C,search:Fj(d),hash:Nj(L)}}function aA(y,s){let k=s.replace(/\/+$/,"").split("/");return y.split("/").forEach(L=>{L===".."?k.length>1&&k.pop():L!=="."&&k.push(L)}),k.length>1?k.join("/"):"/"}function Db(y,s,k,d){return"Cannot include a '"+y+"' character in a manually specified "+("`to."+s+"` field ["+JSON.stringify(d)+"].  Please separate it out to the ")+("`to."+k+"` field. Alternatively you may provide the full path as ")+'a string in <Link to="..."> and the router will parse it for you.'}function Zj(y){return y.filter((s,k)=>k===0||s.route.path&&s.route.path.length>0)}function fA(y,s){let k=Zj(y);return s?k.map((d,L)=>L===k.length-1?d.pathname:d.pathnameBase):k.map(d=>d.pathnameBase)}function MA(y,s,k,d){d===void 0&&(d=!1);let L;typeof y=="string"?L=Lc(y):(L=xc({},y),z1(!L.pathname||!L.pathname.includes("?"),Db("?","pathname","search",L)),z1(!L.pathname||!L.pathname.includes("#"),Db("#","pathname","hash",L)),z1(!L.search||!L.search.includes("#"),Db("#","search","hash",L)));let C=y===""||L.pathname==="",j=C?"/":L.pathname,T;if(j==null)T=k;else{let q=s.length-1;if(!d&&j.startsWith("..")){let V=j.split("/");for(;V[0]==="..";)V.shift(),q-=1;L.pathname=V.join("/")}T=q>=0?s[q]:"/"}let b=Ej(L,T),f=j&&j!=="/"&&j.endsWith("/"),D=(C||j===".")&&k.endsWith("/");return!b.pathname.endsWith("/")&&(f||D)&&(b.pathname+="/"),b}const vA=y=>y.join("/").replace(/\/\/+/g,"/"),Fj=y=>!y||y==="?"?"":y.startsWith("?")?y:"?"+y,Nj=y=>!y||y==="#"?"":y.startsWith("#")?y:"#"+y,mA=["post","put","patch","delete"];new Set(mA);const Gj=["get",...mA];new Set(Gj);/**
 ...
 *
 * @license MIT
 */function ix(){return ix=Object.assign?Object.assign.bind():function(y){for(var s=1;s<arguments.length;s++){var k=arguments[s];for(var d in k)Object.prototype.hasOwnProperty.call(k,d)&&(y[d]=k[d])}return y},ix.apply(this,arguments)}const gA=E.createContext(null),Sa=E.createContext(null),Rb=E.createContext(null),ox=E.createContext({outlet:null,matches:[],isDataRoute:!1});function Xj(y,s){let{relative:k}=s===void 0?{}:s;hx()||z1(!1);let{basename:d,navigator:L}=E.useContext(Sa),{hash:C,pathname:j,search:T}=LA(y,{relative:k}),b=j;return d!=="/"&&(b=j==="/"?d:vA([d,j])),L.createHref({pathname:b,search:T,hash:C})}function hx(){return E.useContext(Rb)!=null}function _b(){return hx()||z1(!1),E.useContext(Rb).location}function xA(y){E.useContext(Sa).static||E.useLayoutEffect(y)}function Pj(){let{isDataRoute:y}=E.useContext(ox);return y?Wj():Yj()}function Yj(){hx()||z1(!1);let y=E.useContext(gA),{basename:s,future:k,navigator:d}=E.useContext(Sa),{matches:L}=E.useContext(ox),{pathname:C}=_b(),j=JSON.stringify(fA(L,k.v7_relativeSplatPath)),T=E.useRef(!1);return xA(()=>{T.current=!0}),E.useCallback(function(f,D){if(D===void 0&&(D={}),!T.current)return;if(typeof f=="number"){d.go(f);return}let q=MA(f,JSON.parse(j),C,D.relative==="path");y==null&&s!=="/"&&(q.pathname=q.pathname==="/"?s:vA([s,q.pathname])),(D.replace?d.replace:d.push)(q,D.state,D)},[s,d,j,C,y])}function LA(y,s){let{relative:k}=s===void 0?{}:s,{future:d}=E.useContext(Sa),{matches:L}=E.useContext(ox),{pathname:C}=_b(),j=JSON.stringify(fA(L,d.v7_relativeSplatPath));return E.useMemo(()=>MA(y,JSON.parse(j),C,k==="path"),[y,j,C,k])}var bA=(function(y){return y.UseBlocker="useBlocker",y.UseRevalidator="useRevalidator",y.UseNavigateStable="useNavigate",y})(bA||{}),SA=(function(y){return y.UseBlocker="useBlocker",y.UseLoaderData="useLoaderData",y.UseActionData="useActionData",y.UseRouteError="useRouteError",y.UseNavigation="useNavigation",y.UseRouteLoaderData="useRouteLoaderData",y.UseMatches="useMatches",y.UseRevalidator="useRevalidator",y.UseNavigateStable="useNavigate",y.UseRouteId="useRouteId",y})(SA||{});function Qj(y){let s=E.useContext(gA);return s||z1(!1),s}function Kj(y){let s=E.useContext(ox);return s||z1(!1),s}function Jj(y){let s=Kj(),k=s.matches[s.matches.length-1];return k.route.id||z1(!1),k.route.id}function Wj(){let{router:y}=Qj(bA.UseNavigateStable),s=Jj(SA.UseNavigateStable),k=E.useRef(!1);return xA(()=>{k.current=!0}),E.useCallback(function(L,C){C===void 0&&(C={}),k.current&&(typeof L=="number"?y.navigate(L):y.navigate(L,ix({fromRouteId:s},C)))},[y,s])}function $j(y,s){y==null||y.v7_startTransition,y==null||y.v7_relativeSplatPath}function eI(y){let{basename:s="/",children:k=null,location:d,navigationType:L=O2.Pop,navigator:C,static:j=!1,future:T}=y;hx()&&z1(!1);let b=s.replace(/^\/*/,"/"),f=E.useMemo(()=>({basename:b,navigator:C,static:j,future:ix({v7_relativeSplatPath:!1},T)}),[b,T,C,j]);typeof d=="string"&&(d=Lc(d));let{pathname:D="/",search:q="",hash:V="",state:oe=null,key:Y="default"}=d,ae=E.useMemo(()=>{let X=kA(D,b);return X==null?null:{location:{pathname:X,search:q,hash:V,state:oe,key:Y},navigationType:L}},[b,D,q,V,oe,Y,L]);return ae==null?null:E.createElement(Sa.Provider,{value:f},E.createElement(Rb.Provider,{children:k,value:ae}))}new Promise(()=>{});/**
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const mI=y=>y.replace(/([a-z0-9])([A-Z])/g,"$1-$2").toLowerCase().trim(),l=(y,s)=>{const k=E.forwardRef(({color:d="currentColor",size:L=24,strokeWidth:C=2,absoluteStrokeWidth:j,className:T="",children:b,...f},D)=>E.createElement("svg",{ref:D,...vI,width:L,height:L,stroke:d,strokeWidth:j?Number(C)*24/Number(L):C,className:["lucide",`lucide-${mI(y)}`,T].join(" "),...f},[...s.map(([q,V])=>E.createElement(q,V)),...Array.isArray(b)?b:[b]]));return k.displayName=`${y}`,k};/**
 * @license lucide-react v0.300.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
